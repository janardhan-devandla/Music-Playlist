...
class App extends Component {
  ...
  deleteHandler = delId => {
    ...
    const newData = tracksList.filter(each => each.id !== delId)
    this.setState({tracksList: newData})
  }

  render() {
    const {tracksList, searchInput} = this.state
    const filterData = tracksList.filter(each =>
      ...
      <div className="bg-container">
        ...
        <div className="main-container">
          <div className="search-container">
            <h1 className="songs-title">Songs Playlist</h1>
            <input
              type="search"
              placeholder="Search"
              value={searchInput}
          ...
          <ul>
            {filterIsTrue ? (
              <p className="no-data">No Songs Found</p>
            ) : (
              filterData.map(eachList => (
                <MusicPlayList
                  playList={eachList}
                  key={eachList.id}
                  deleteButton={this.deleteHandler}
                />
              ))
            )}
          </ul>
